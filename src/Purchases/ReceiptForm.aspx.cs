...
public partial class Purchases_ReceiptForm : UICulturePage
{
    ...
    private int TypeTax
    {
        get
        {
            if (Session["TypeTax_Invoice" + this.WinID] == null)
            {
                ...
                Session["TypeTax_Invoice" + this.WinID] = dc.usp_Company_Select().FirstOrDefault().TypeTaxPurchase;
            }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Page.IsPostBack)
            {
                ...
                WebControl wcICausedPostBack1 = (WebControl)GetControlThatCausedPostBack1(sender as Page);
    ...
    protected Control GetControlThatCausedPostBack1(Page page)
    {
    ...
    private void FilterByBranchAndCurrency()
    {
        try
        {
            ...
            acCashAccount.ContextKey = this.MyContext.CurrentCulture.ToByte().ToExpressString() + "," + acBranch.Value + "," + ddlCurrency.SelectedValue + "," + null + ",false,true";
        ...
        if (!this.EditMode)
        {
            ...
            if (this.Receipt_ID > 0)
            {
                foreach (DataRow r in this.dtItems.Rows)
                {
                    ...
                    Detail_ID = dc.usp_ReceiptDetailsIncludeTax_Insert(this.Receipt_ID, r["Store_ID"].ToInt(), r["Item_ID"].ToInt(), r["UnitCost"].ToDecimal(), r["Quantity"].ToDecimal(), r["Uom_iD"].ToInt(), r["Batch_ID"].ToNullableInt(), r["Tax_ID"].ToNullableInt(), r["PercentageDiscount"].ToDecimalOrDefault(), r["CashDiscount"].ToDecimalOrDefault(), r["TotalTax"].ToDecimalOrDefault(), r["Notes"].ToExpressString(), r["QtyInNumber"].ToDecimalOrDefault(), r["Capacity"].ToString(), r["ItemDescription"].ToString(), r["IDCodeOperation"].ToString(), r["Policy"].ToString(), r["ReceiptDate"].ToDate(), r["Capacities"].ToString(), r["UnitCostEvaluate"].ToDecimalOrDefault(),
                        false,false,false);
            ...
            if (Result > 0)
            {
                foreach (DataRow r in this.dtItems.Rows)
                {
                    ...
                    if (r.RowState == DataRowState.Modified)
                    {
                        ...
                        Detail_ID = dc.usp_ReceiptDetailsIncludeTax_Update(r["ID"].ToInt(), r["Store_ID"].ToInt(), r["Item_ID"].ToInt(), r["UnitCost"].ToDecimal(), r["Quantity"].ToDecimal(), r["Uom_iD"].ToInt(), r["Batch_ID"].ToNullableInt(), r["Tax_ID"].ToNullableInt(), r["PercentageDiscount"].ToDecimalOrDefault(), r["CashDiscount"].ToDecimalOrDefault(), r["TotalTax"].ToDecimalOrDefault(), r["Notes"].ToExpressString(), r["QtyInNumber"].ToDecimalOrDefault(), r["Capacity"].ToString(), r["ItemDescription"].ToString(), r["IDCodeOperation"].ToString(), r["Policy"].ToString(), r["ReceiptDate"].ToDate(), r["Capacities"].ToString(), r["UnitCostEvaluate"].ToDecimalOrDefault(),
                            false, false, false);
    ...
    private void InsertOperation()
    {
        ...
        //CostCenter 
        dc.usp_CostCenterOperation_Insert(acBranch.Value.ToNullableInt(), acCostCenter.Value.ToNullableInt(), txtOperationDate.Text.ToDate(), 
            this.GrossTotal * ratio, this.Receipt_ID, DocumentsTableTypes.Receipt.ToInt(), txtNotes.Text);
        this.InsertCashOut();
    }
    ...
    private void FillReceipt()
    {
        ...
        if (!string.IsNullOrEmpty(idGene))
        {


            var IDG = int.Parse(idGene);
            var receipt = dc.usp_Receipt_SelectByID(IDG).FirstOrDefault();
            ...
            var lstItems = dc.usp_ReceiptDetailsWithDescription_Select(IDG).ToList();
            this.dtItems = lstItems.CopyToDataTable();

            this.dtTaxes = dc.usp_DocuemntTaxes_Select(DocumentsTableTypes.Receipt.ToInt(), IDG, false).CopyToDataTable();
            this.BindItemsGrid();
            gvTaxes.DataSource = this.dtTaxes;
            gvTaxes.DataBind();
        }
    ...
    private void Calculate()
    {
        ...
        foreach (DataRow r in this.dtItems.Rows)
        {
            ...
            if (r["TaxOnReceiptType"] != DBNull.Value && r["Tax_ID"] != DBNull.Value)
            {
                ...
                if (r["TaxOnReceiptType"].ToExpressString() == "C")
                {
                    this.dtAllTaxes.Rows.Add(r["TaxPurchaseAccountID"].ToInt(), r["TaxOnReceiptType"].ToExpressString(), 
                        0, r["TotalTax"].ToDecimal());
                    this.TotalCreditTax += r["TotalTax"].ToDecimal();
                    //r["TotalTax"] = r["TotalTax"].ToDecimal() * -1;
                }
                else
