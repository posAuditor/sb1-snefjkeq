...
public partial class Purchases_Receipt : UICulturePage
{
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            UserMessages.Message(null, Resources.UserInfoMessages.OperationSuccess, PageLinks.Receipt + "?ID=" + this.Receipt_ID.ToExpressString(), PageLinks.ReceiptsList, PageLinks.Receipt);
        }
        catch (Exception ex)
        {
            Logger.LogError(Resources.UserInfoMessages.OperationFailed, ex);
    ...
    private bool Save(bool IsApproving, System.Data.Common.DbTransaction trans)
    {
        ...
        else
        {
            int Result = dc.usp_Receipt_Update(this.Receipt_ID, acBranch.Value.ToNullableInt(), ddlCurrency.SelectedValue.ToInt(), txtRatio.Text.ToDecimalOrDefault(), txtOperationDate.Text.ToDate(),
                                                    ...
                                                    lblTotalDiscount.Text.ToDecimalOrDefault(), lblTotalTax.Text.ToDecimalOrDefault(), lblGrossTotal.Text.ToDecimalOrDefault(), txtFirstPaid.Text.ToDecimalOrDefault(), txtUserRefNo.Text, this.DocRandomString,
                                                    acCashAccount.Value.ToNullableInt(), acdrivers.Value.ToNullableInt());
            if (Result > 0)
            {
                foreach (DataRow r in this.dtItems.Rows)
                {
        ...
        UserMessages.Message(null, Resources.UserInfoMessages.OperationSuccess + Serial, PageLinks.Receipt + "?ID=" + this.Receipt_ID.ToExpressString(), PageLinks.ReceiptsList, PageLinks.Receipt);
        return true;
    }
