...
public partial class Accounting_JournalEntry : UICulturePage
{
    ...
    protected void btnAddDetail_click(object sender, EventArgs e)
    {
        try
        {
            ...
            var AcDCount = dc.fun_AccountHaveChild(acAccount.Value.ToInt());
            if (AcDCount > 0)
            {
                UserMessages.Message(null, "الحساب  لا يمكن ان يكون حساب اب", string.Empty);
            ...
            r["IsTaxFound"] = chkIsTaxFound.Checked;
            r["TypeTax"] = dc.usp_Company_Select().FirstOrDefault().TypeTax.ToIntOrDefault();
            r["Tax_ID"] = acItemTax.Value.ToInt();
            if (chkIsTaxFound.Checked)
            {
                if (acItemTax.HasValue)
                {
                    var tx = dc.usp_Taxes_Select(acItemTax.Value.ToIntOrDefault(), string.Empty).FirstOrDefault();
                    if (tx != null)
                    {

                        decimal taxInclude = r["TypeTax"].ToIntOrDefault() == 2 ? (Decimal.Divide((tx != null ? tx.PercentageValue.Value : 0), (100M + (tx != null ? tx.PercentageValue.Value : 0)))) : Decimal.Divide((tx != null ? tx.PercentageValue.Value : 0), (100M));


                        r["PercentTaxValue"] = ((r["DebitAmount"].ToDecimalOrDefault() > 0 ? r["DebitAmount"].ToDecimalOrDefault() : r["CreditAmount"].ToDecimalOrDefault())) * taxInclude;
                    }
                }
            }
            else
            ...
            if (chkIsTaxFound.Checked)
            {
                ...
                rTax = this.dtOperationDetails.NewRow();
                rTax["ID"] = this.dtOperationDetails.GetID("ID");
                //}
                //else
                //{
                //    rTax = this.dtOperationDetails.Select("ID=" + this.EditID)[0];
                //}

                decimal taxInclude = 0;
                if (acItemTax.HasValue)
                {
                    var tx = dc.usp_Taxes_Select(acItemTax.Value.ToIntOrDefault(), string.Empty).FirstOrDefault();
                    if (tx != null)
                    {
                        ...
                        taxInclude = r["TypeTax"].ToIntOrDefault() == 2 ? (Decimal.Divide((tx != null ? tx.PercentageValue.Value : 0), (100M + (tx != null ? tx.PercentageValue.Value : 0)))) : Decimal.Divide((tx != null ? tx.PercentageValue.Value : 0), (100M));



                    }

                    rTax["Account_ID"] = tx.SalesAccountID;
                    rTax["AccountName"] = tx.SalesAccountName;
                }

                rTax["DebitAmount"] = ((r["DebitAmount"].ToDecimalOrDefault() > 0 ? r["DebitAmount"].ToDecimalOrDefault() : 0)) * taxInclude;
                rTax["CreditAmount"] = ((r["CreditAmount"].ToDecimalOrDefault() > 0 ? r["CreditAmount"].ToDecimalOrDefault() : 0)) * taxInclude;
                rTax[this.DebitColumnName] = rTax["DebitAmount"].ToDecimalOrDefault();
                rTax[this.CreditColumnName] = rTax["CreditAmount"].ToDecimalOrDefault();
                rTax["Description"] = txtDetailNotes.Text;
                rTax["CostCenter_ID"] = acDetailCostCenter.Value.ToIntOrDBNULL();

                rTax["CostCenterName"] = acDetailCostCenter.Text;
                if (txtDate.Text.ToDate() != null)
                {
                    rTax["OperDate"] = txtDate.Text.ToDate();
                }
                rTax["VendorsSecond_ID"] = acVendorsSecond.Value.ToIntOrDBNULL();
                rTax["VendorsSecondname"] = acVendorsSecond.Text;

                rTax["IsTaxFound"] = false;
                rTax["TypeTax"] = -1;
                // rTax["Tax_ID"] = null;
                rTax["PercentTaxValue"] = 0;
                rTax["Parent_ID"] = r["Parent_ID"] = (new Random()).Next(80000952, 999999952);
                this.dtOperationDetails.Rows.Add(rTax);
            ...
            else
            {
                try
                {
                    if (r["Parent_ID"].ToExpressString() != "0")
                    {
                        DataRow dr = this.dtOperationDetails.Select("Parent_ID =" + r["Parent_ID"] + " And ID <>" + this.EditID)[0];
                        if (dr != null)
                        {
                            dr.Delete();
                            r["Parent_ID"] = DBNull.Value;
                            r["PercentTaxValue"] = DBNull.Value;
                            r["IsTaxFound"] = false;
                            r["TypeTax"] = DBNull.Value;
                        }
                    }
    ...
    private void LoadControls()
    {
        ...
        acItemTax.ContextKey = DocumentsTableTypes.Invoice.ToInt().ToExpressString() + ",false";
    }
        ...
        if (!this.EditMode)
        {
            ...
            if (this.Operation_ID > 0)
            {
                foreach (DataRow r in this.dtOperationDetails.Rows)
                {
                    ...
                    dc.usp_JournalEntryDetailsWithVendorsSecond_ID_Insert(this.Operation_ID, r["Account_ID"].ToInt(),
                        r[this.DebitColumnName].ToDecimal() * txtRatio.Text.ToDecimal(), r[this.CreditColumnName].ToDecimal() * txtRatio.Text.ToDecimal(),
                        r[this.DebitColumnName].ToDecimal() * ZERO_Foreign, r[this.CreditColumnName].ToDecimal() * ZERO_Foreign,
                        r["Description"].ToExpressString(), r["CostCenter_ID"].ToNullableInt(), r["OperDate"].ToDate(),
                        r["VendorsSecond_ID"].ToNullableInt(),
                        r["IsTaxFound"].ToBooleanOrDefault(),
                        r["TypeTax"].ToIntOrDefault(),
                        r["PercentTaxValue"].ToDecimalOrDefault(),
                        r["Tax_ID"].ToIntOrDefault(),
                        r["Parent_ID"].ToIntOrDefault(),
                        string.Empty
                        );
                    if (IsApproving)
                    {
        ...
        else
        {
            ...
            if (Result > 0)
            {
                foreach (DataRow r in this.dtOperationDetails.Rows)
                {
                    ...
                    {
                        dc.usp_JournalEntryDetailsWithVendorsSecond_ID_Insert(this.Operation_ID, r["Account_ID"].ToInt(),
                            r[this.DebitColumnName].ToDecimal() * txtRatio.Text.ToDecimal(), r[this.CreditColumnName].ToDecimal() * txtRatio.Text.ToDecimal(), r[this.DebitColumnName].ToDecimal() * ZERO_Foreign,
                            r[this.CreditColumnName].ToDecimal() * ZERO_Foreign, r["Description"].ToExpressString(), r["CostCenter_ID"].ToNullableInt(), r["OperDate"].ToDate(),
                            r["VendorsSecond_ID"].ToNullableInt(),
                            r["IsTaxFound"].ToBooleanOrDefault(),
                            r["TypeTax"].ToIntOrDefault(),
                            r["PercentTaxValue"].ToDecimalOrDefault(),
                            r["Tax_ID"].ToIntOrDefault(),
                        r["Parent_ID"].ToIntOrDefault(),string.Empty
                            );
                    }
                    if (r.RowState == DataRowState.Modified)
                    {
                        dc.usp_JournalEntryDetailsWithVendorsSecond_ID_Update(r["ID"].ToInt(), r["Account_ID"].ToInt(),
                            r[this.DebitColumnName].ToDecimal() * txtRatio.Text.ToDecimal(), r[this.CreditColumnName].ToDecimal() * txtRatio.Text.ToDecimal(),
                            r[this.DebitColumnName].ToDecimal() * ZERO_Foreign, r[this.CreditColumnName].ToDecimal() * ZERO_Foreign, r["Description"].ToExpressString(), r["CostCenter_ID"].ToNullableInt(),
                            r["OperDate"].ToDate(),
                            r["VendorsSecond_ID"].ToNullableInt(),
                            r["IsTaxFound"].ToBooleanOrDefault(),
                            r["TypeTax"].ToIntOrDefault(),
    ...
    protected void gvDetails_SelectedIndexChanging1(object sender, GridViewSelectEventArgs e)
    {
        try
        {
            ...
            chkIsTaxFound.Checked = r["IsTaxFound"].ToBooleanOrDefault();
