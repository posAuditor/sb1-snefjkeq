...
public partial class Accounting_COAOrderAccount : UICulturePage
{
    ...
    protected void btnMerge_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            var cos = dc.ChartOfAccounts.Where(c => c.ID == acAccountFrom.Value.ToInt()).FirstOrDefault();
            if (cos != null)
            {
                cos.DisplayOrder = txtOrderNumber.Text.ToInt();
                dc.SubmitChanges();
            }

          //  var trasnferAccount = dc.usp_TransferAccountGroup(acAccountTo.Value.ToInt(), acAccountFrom.Value.ToInt());
            //{
            //    case 1:
        ...
        {
            UserMessages.Message(null, Resources.UserInfoMessages.OperationFailed, string.Empty);
            Logger.LogError(Resources.UserInfoMessages.OperationFailed, ex);
        }
