...
public partial class Accounting_GeneralJournalList : UICulturePage
{
    ...
    private void FillJournalEntriesList()
    {
        ...
        this.dtGeneralJournalList = dc.usp_GeneralJournal_Select(acBranch.Value.ToNullableInt(), txtDateFromSrch.Text.ToString().ToDate(), txtDateToSrch.Text.ToString().ToDate(), txtSerialsrch.TrimmedText, Currency_ID, acOperationType.Value.ToNullableInt(), MyContext.CurrentCulture.ToByte(), acBrancheGroup.Value.ToNullableInt()).CopyToDataTable();
        gvGeneralJournalList.DataSource = this.dtGeneralJournalList;
        gvGeneralJournalList.DataBind();
    }
    ...
    protected void acBrancheGroup_SelectedIndexChanged(object sender, AutoCompleteEventArgs e)
    {

        acBranch.ContextKey = acBrancheGroup.Value.ToExpressString();
