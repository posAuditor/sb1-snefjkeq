...
public partial class Accounting_ChartOfAccountsVendors : UICulturePage
{
    ...
    protected void btnSaveAccount_click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (hfEditAccount.Value != string.Empty)
            {
                ...
                result = dc.usp_ChartOfAccountException_Update(hfEditAccount.Value.ToInt(), txtAccountName.TrimmedText, 
                    txtAccountNameEN.TrimmedText, acParentAccount.Value.ToInt(), false, acBranch.Value.ToNullableInt(),
                    ddlCurrency.SelectedValue.ToInt(), txtRatio.Text.ToDecimalOrDefault(),
                    txtOpenBalance.Text.ToDecimalOrDefault(), txtStartFrom.Text.ToDate(), acOppsiteAccount.Value.ToInt(),false,null,false);
                if (result == -2)
                {
                    UserMessages.Message(this.MyContext.PageData.PageTitle, Resources.UserInfoMessages.AccountNameAlreadyExists, string.Empty);
                    trans.Rollback();
    ...
    private void FillCOATree()
    {
       ...
       // DataTable dtCOA = dc.usp_ChartOfAccountTree_Select(this.MyContext.CurrentCulture.ToByte(), MyContext.UserProfile.Branch_ID).Where(x => x.ParentId == COA.Vendors.ToInt()).CopyToDataTable();
        DataTable dtCOA = dc.usp_ChartOfAccountTreeCustom_Select(this.MyContext.CurrentCulture.ToByte(), MyContext.UserProfile.Branch_ID, COA.Vendors.ToInt()).CopyToDataTable();
        ...
        foreach (DataRow dr in dtCOA.Rows)
        {
            node = new TreeNode(this.GetNodeText(dr.Field<string>("Account"), dr.Field<string>("Name_ar"), dr.Field<string>("Name_en"), dr["Total"].ToExpressString(), dr.Field<int>("AccountLevel"), dr["ID"].ToExpressString(), dr["IsSystem"].ToBoolean(), dr["ModuleRelated"].ToBoolean()), dr["ID"].ToExpressString());
