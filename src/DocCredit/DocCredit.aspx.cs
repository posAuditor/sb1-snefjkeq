...
public partial class DocCredit_DocCredits : UICulturePage
{
    ...
    private int DocCredit_ID
    {
        get
        {
            if (ViewState["DocCredit_ID"] == null) return 0;
            return (int)ViewState["DocCredit_ID"];
        }

        set
        {
            ViewState["DocCredit_ID"] = value;
        }
    }

    private int DocCreditChartOfAccount_ID
    {
        get
        {
            if (ViewState["DocCreditChartOfAccount_ID"] == null) return 0;
            return (int)ViewState["DocCreditChartOfAccount_ID"];
        }

        set
        {
            ViewState["DocCreditChartOfAccount_ID"] = value;
        }
    }
    ...
    private bool PopupMode
    {
        get
        {
            if (ViewState["PopupMode"] == null) return false;
            return (bool)ViewState["PopupMode"];
        }

        set
        {
            ViewState["PopupMode"] = value;
        }
    }
    ...
    private DataTable dtItems
    {
        get
        {
            ...
            {
                Session["dtItems_Receipt" + this.WinID] = dc.usp_ReceiptDetailsWithDescription_Select(null).CopyToDataTable(); //dc.usp_ReceiptDetails_Select(null).CopyToDataTable();
            }
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            UserMessages.Message(null, Resources.UserInfoMessages.OperationSuccess, PageLinks.DocumentryCredit + "?ID=" + this.Receipt_ID.ToExpressString(), PageLinks.DocumentryCreditList, PageLinks.DocumentryCredit);
        }
        catch (Exception ex)
        {
            Logger.LogError(Resources.UserInfoMessages.OperationFailed, ex);
    ...
    protected void btnReceive_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (txtReceivingDate.Text.ToDate() < txtOperationDate.Text.ToDate())
            {
                UserMessages.Message(null, Resources.UserInfoMessages.ReceDateLessOperationDate, string.Empty);
                trans.Rollback();
                return;
            }
            foreach (DataRow r in this.dtItems.Rows)
            {
                ...
                {
                    if (!this.InsertICJ(r["ID"].ToInt(), r))
                    {
                        trans.Rollback();
                        return;
                    }
            ...
            decimal InventoryCost = dc.fun_GetAccountBalance(this.DocCreditChartOfAccount_ID, null, acBranch.Value.ToNullableInt()).Value;
            decimal ForeignInventoryCost = dc.fun_GetAccountBalanceInForeign(this.DocCreditChartOfAccount_ID, null, acBranch.Value.ToNullableInt()).Value;

            int Operation_ID = dc.usp_Operation_Insert(acBranch.Value.ToNullableInt(), txtReceivingDate.Text.ToDate(), ref serial, DocStatus.Approved.ToByte(), OperationTypes.ReceiveDocumentryCredit.ToInt(), ddlCurrency.SelectedValue.ToInt(), InventoryCost, ForeignInventoryCost, ratio, null);

            //Doc Credit Account
            dc.usp_OperationDetails_Insert(Operation_ID, this.DocCreditChartOfAccount_ID, 0, InventoryCost, 0, ForeignInventoryCost, null, this.Receipt_ID, DocumentsTableTypes.Receipt.ToInt());

            //Inventory
            dc.usp_OperationDetails_Insert(Operation_ID, company.InventoryAccount_ID, InventoryCost, 0, ForeignInventoryCost, 0, null, this.Receipt_ID, DocumentsTableTypes.Receipt.ToInt());

            dc.usp_DocumentryCredit_Update(this.Receipt_ID, null, null, null, txtReceivingDate.Text.ToDate(), true);

            dc.usp_DocumentryCreditExpenses_Distribute(this.Receipt_ID);

            dc.usp_ICJ_NegativeRefill(txtReceivingDate.Text.ToDate(), acBranch.Value.ToNullableInt(), DocumentsTableTypes.Receipt.ToInt(), this.Receipt_ID);
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            doc.Load(Server.MapPath("~\\Reports\\DocCredit_Print.rpt"));
            ...
            Response.Redirect(PageLinks.Print + "?File=" + doc.ExportToPDF(acBranch.Value.ToNullableInt(), "DocumentryCredit"), false);
        }
        catch (Exception ex)
        {
            Logger.LogError(Resources.UserInfoMessages.OperationFailed, ex);
    ...
    private void FilterByBranchAndCurrency()
    {
        try
        {
           ...
           // acBank.ContextKey = this.MyContext.CurrentCulture.ToByte().ToExpressString() + "," + acBranch.Value + "," + ddlCurrency.SelectedValue + "," + COA.Banks.ToInt().ToExpressString() + ",false";
            acBank.ContextKey = this.MyContext.CurrentCulture.ToByte().ToExpressString() + "," + acBranch.Value + "," + ddlCurrency.SelectedValue + ",,false";
    ...
    private void SetEditMode()
    {
        ...
        this.PopupMode = (Request["ViewInPopUpMode"] != null);
    }
    ...
    private bool Save(bool IsApproving, System.Data.Common.DbTransaction trans)
    {
        ...
        if (txtCoverValue.Text.ToDecimal() > this.GrossTotal)
        {
            UserMessages.Message(null, Resources.UserInfoMessages.CoverValueGreaterThanTotal, string.Empty);
            trans.Rollback();
            return false;
        }
        ...
        byte EntryType = 1;
        int Serial_ID = DocSerials.DocumentryCredit.ToInt();
        ...
        if (!this.EditMode)
        {
            ...
            this.Receipt_ID = dc.usp_Receipt_Insert(acBranch.Value.ToNullableInt(), ddlCurrency.SelectedValue.ToInt(), txtRatio.Text.ToDecimalOrDefault(), txtOperationDate.Text.ToDate(),
                                                     ...
                                                     txtCashDiscount.Text.ToDecimalOrDefault(), lblTotalDiscount.Text.ToDecimalOrDefault(), lblTotalTax.Text.ToDecimalOrDefault(), lblGrossTotal.Text.ToDecimalOrDefault(), 0, null, txtUserRefNo.Text, this.DocRandomString, EntryType, null, null, null);
            if (this.Receipt_ID > 0)
            {
                this.DocCredit_ID = dc.usp_DocumentryCredit_Insert(this.Receipt_ID, txtDocCreditName.TrimmedText, acBank.Value.ToInt(), txtCoverValue.Text.ToDecimalOrDefault(), txtReceivingDate.Text.ToDate());
                if (DocCredit_ID == -2)
                {
        ...
        else
        {
            int Result = dc.usp_Receipt_Update(this.Receipt_ID, acBranch.Value.ToNullableInt(), ddlCurrency.SelectedValue.ToInt(), txtRatio.Text.ToDecimalOrDefault(), txtOperationDate.Text.ToDate(),
                                                    ...
                                                    lblTotalDiscount.Text.ToDecimalOrDefault(), lblTotalTax.Text.ToDecimalOrDefault(), lblGrossTotal.Text.ToDecimalOrDefault(), 0, txtUserRefNo.Text, this.DocRandomString, null, null);
            if (Result > 0)
            {
                int DocCreditRowCount = dc.usp_DocumentryCredit_Update(this.Receipt_ID, txtDocCreditName.TrimmedText, acBank.Value.ToInt(), txtCoverValue.Text.ToDecimalOrDefault(), txtReceivingDate.Text.ToDate(), false);
                if (DocCreditRowCount == -2)
                {
                    UserMessages.Message(null, Resources.UserInfoMessages.NameAlreadyExists, string.Empty);
                    trans.Rollback();
                    return false;
        ...
        UserMessages.Message(null, Resources.UserInfoMessages.OperationSuccess + Serial, PageLinks.DocumentryCredit + "?ID=" + this.Receipt_ID.ToExpressString(), PageLinks.DocumentryCreditList, PageLinks.DocumentryCredit);
        return true;
    }
    ...
    private bool InsertOperation()
    {
        ...
        int? DocCreditAccount_ID = 0;

        //حساب الاعتماد المستندى
        int DocResult = dc.usp_DocumentryCredit_Approve(this.Receipt_ID, txtDocCreditName.TrimmedText, acBranch.Value.ToNullableInt(), ddlCurrency.SelectedValue.ToInt(), txtRatio.Text.ToDecimal(), txtCoverValue.Text.ToDecimalOrDefault(), txtOperationDate.Text.ToDate(), acBank.Value.ToInt(), ref DocCreditAccount_ID);
        if (DocResult == -2)
        {
            UserMessages.Message(null, Resources.UserInfoMessages.NameAlreadyExists, string.Empty);
            return false;
        ...
        int Result = dc.usp_Operation_Insert(acBranch.Value.ToNullableInt(), txtOperationDate.Text.ToDate(), ref serial, DocStatus.Approved.ToByte(), OperationTypes.DocumentryCredit.ToInt(), ddlCurrency.SelectedValue.ToInt(), (this.Total + this.TotalDebitTax) * ratio, (this.Total + this.TotalDebitTax), ratio, txtNotes.Text);
        ...
        //Doc Credit Account
        dc.usp_OperationDetails_Insert(Result, DocCreditAccount_ID, (this.Total) * ratio, 0, (this.Total), 0, null, this.Receipt_ID, DocumentsTableTypes.Receipt.ToInt());
    ...
    private bool InsertICJ(int Detail_ID, DataRow row)
    {
        ...
        result = dc.usp_ICJ_Receipt(txtReceivingDate.Text.ToDate(), row["Quantity"].ToDecimal(), row["Item_ID"].ToInt(), row["Uom_ID"].ToInt(), ItemTotal, row["Batch_ID"].ToNullableInt(), row["Store_ID"].ToInt(), acBranch.Value.ToNullableInt(), DocumentsTableTypes.Receipt.ToInt(), this.Receipt_ID, Detail_ID, ref IsService);
    ...
    private void FillReceipt()
    {
        ...
        acBank.Value = DocCredit.BankChartOfAccount_ID.ToExpressString();
        txtDocCreditName.Text = DocCredit.DocName;
        txtCoverValue.Text = DocCredit.CoverValue.ToExpressString();
        if (DocCredit.ReceivingDate.HasValue) txtReceivingDate.Text = DocCredit.ReceivingDate.Value.ToString("d/M/yyyy");
        if (DocCredit.ChartOfAccount_ID.HasValue) this.DocCreditChartOfAccount_ID = DocCredit.ChartOfAccount_ID.Value;
        this.DocCredit_ID = DocCredit.ID;
        ...
        btnReceive.Visible = (receipt.DocStatus_ID == 2) && MyContext.PageData.IsApprove && !DocCredit.IsReceived.Value;
        ...
        this.dtItems = dc.usp_ReceiptDetailsWithDescription_Select(this.Receipt_ID).CopyToDataTable();
    ...
    private void CheckSecurity()
    {
        ...
        btnReceive.Visible = MyContext.PageData.IsApprove && EditMode;
