...
public partial class Sales_QuotaList : UICulturePage
{
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            string SaveName = Request.PathInfo == "/Quota" ? "Quota" : "Quota";
            ...
            doc.Load(Server.MapPath("~\\Reports\\InvoiceReport.rpt"));
    ...
    {
        byte EntryType = Request.PathInfo == "/Quota" ? (byte)2 : (byte)0;
        lnkadd.NavigateUrl = PageLinks.Quota;
        //gvInvoicesList.Columns[10].Visible = Request.PathInfo != "/Quota";
       // gvInvoicesList.Columns[11].Visible = Request.PathInfo == "/Quota";
        //gvInvoicesList.Columns[12].Visible = Request.PathInfo == "/Quota";
        divHasBill.Visible = Request.PathInfo != "/Quota";
        btnPrintList.Visible = Request.PathInfo == "/Quota" && MyContext.PageData.IsPrint;
        int? Currency_ID = ddlCurrency.SelectedIndex == 0 ? (int?)null : ddlCurrency.SelectedValue.ToInt();
        byte? DocStatus_ID = ddlStatus.SelectedIndex == 0 ? (byte?)null : ddlStatus.SelectedValue.ToByte();
        bool? HasInvoice = ddlIsHasBill.SelectedIndex == 0 ? (bool?)null : ddlIsHasBill.SelectedValue.ToBoolean();
        var lstInvoices =
            dc.usp_InvoiceQuota_Select(acBranch.Value.ToNullableInt(), Currency_ID, txtSerialsrch.TrimmedText,
