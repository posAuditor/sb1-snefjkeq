...
public partial class Sales_InvoicesListEntry : UICulturePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            gvInvoicesList.FormatNumber = MyContext.FormatNumber;
            if (!Page.IsPostBack)
            {
    ...
    {
        byte EntryType = (byte)3;
        lnkadd.NavigateUrl = PageLinks.InvoiceFormEntry;
        ...
        bool? HasInvoice = ddlIsHasBill.SelectedIndex == 0 ? (bool?)null : ddlIsHasBill.SelectedValue.ToBoolean();
        var lstInvoices =
            dc.usp_InvoiceEntry_Select(acBranch.Value.ToNullableInt(), Currency_ID, txtSerialsrch.TrimmedText,
                acCustomerName.Value.ToNullableInt(), txtDateFromSrch.Text.ToDate(), txtDateToSrch.Text.ToDate(),
                txtUserRefNo.Text, DocStatus_ID, MyContext.CurrentCulture.ToByte(), HasInvoice, EntryType).ToList().Where(x=>x.IsPos==null);
        //foreach (var item in lstInvoices)
        //{
        //    item.PageName = item.PageName.Replace("Invoice", "InvoiceFormEntry");
        //    item.ToInvoicePage = item.ToInvoicePage.Replace("Invoice", "InvoiceFormEntry");
        //}
        this.dtInvoicesList = lstInvoices.Where(x=>x.EntryType ==3) .CopyToDataTable();
