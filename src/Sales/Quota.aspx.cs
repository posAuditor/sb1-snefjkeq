...
public partial class Sales_Quota : UICulturePage
{
    ...
    private DataTable dtItems
    {
        get
        {
            if (Session["dtItems_SalesORDER" + this.WinID] == null)
            {
                ...
                Session["dtItems_SalesORDER" + this.WinID] = dc.usp_InvoiceQuotaDetailsWithDescription_Select(null).CopyToDataTable();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                this.LoadControls();
                this.SetDefaults();
                if (EditMode) this.FillInvoiceQuota();
               // VisibiliyControl();
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            doc.Load(Server.MapPath("~\\Reports\\InvoiceQ_PrintEn.rpt"));
            ...
            Response.Redirect(PageLinks.Print + "?File=" + doc.ExportToPDF(acBranch.Value.ToNullableInt(), "Quota"), false);
        }
        catch (Exception ex)
        {
            Logger.LogError(Resources.UserInfoMessages.OperationFailed, ex);
    ...
    protected void btnPrintArabic_Click(object sender, EventArgs e)
    {
        try
        {

            ReportDocument doc = new ReportDocument();
            doc.Load(Server.MapPath("~\\Reports\\InvoiceQ_Print.rpt"));
            doc.SetParameterValue("@Invoice_ID", this.Invoice_ID);
    ...
    protected void btnPrint_Dev_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("~/Report_Dev/PrintQuotaDev.aspx?Invoice_ID=" + this.Invoice_ID);
        }
        catch (Exception)
        {
    ...
    protected void txtDateFinish_TextChanged(object sender, EventArgs e)
    {
        if (txtDateFinish.Text != "")
        {
            var d1 = txtDateFinish.Text.ToDate();
            var d2 = (d1.Value - DateTime.Now.Date).TotalDays;

            txtDeleveyDay.Text = d2.ToExpressString();
    ...
    private bool Save(bool IsApproving, System.Data.Common.DbTransaction trans)
    {
        ...
        byte EntryType = 0;
        int Serial_ID = DocSerials.Quota.ToInt();
        int? SalesOrderID = null;
        if (!this.EditMode)
        {
            //TODO INVOICE contact Mesure
            this.Invoice_ID = dc.usp_InvoiceQuota_Insert(acBranch.Value.ToNullableInt(), ddlCurrency.SelectedValue.ToInt(), txtRatio.Text.ToDecimalOrDefault(), txtOperationDate.Text.ToDate(),
                                                     ...
                                                     txtCashDiscount.Text.ToDecimalOrDefault(), txtAdditionals.Text.ToDecimalOrDefault(), lblTotalDiscount.Text.ToDecimalOrDefault(), lblTotalTax.Text.ToDecimalOrDefault(), lblGrossTotal.Text.ToDecimalOrDefault(), txtFirstPaid.Text.ToDecimalOrDefault(), null, txtUserRefNo.Text, this.DocRandomString, EntryType, SalesOrderID, acSalesRep.Value.ToNullableInt(), acCashAccount.Value.ToNullableInt(), null, txtDateFinish.Text.ToDate(), txtDeleveyDay.Text.ToNullableInt(), txtPaymentMethode.Text, txtAccountNumber.Text);
            if (this.Invoice_ID > 0)
            {
                foreach (DataRow r in this.dtItems.Rows)
                {
                    ...
                    dc.usp_InvoiceQuotaDetailsWithDesciption_Insert(this.Invoice_ID, r["Store_ID"].ToInt(), r["Item_ID"].ToInt(), r["UnitCost"].ToDecimal(), r["Quantity"].ToDecimal(), r["Uom_iD"].ToInt(), r["Batch_ID"].ToNullableInt(), r["Tax_ID"].ToNullableInt(), r["PercentageDiscount"].ToDecimalOrDefault(), r["CashDiscount"].ToDecimalOrDefault(), r["TotalTax"].ToDecimalOrDefault(), r["Notes"].ToExpressString(), r["QtyInNumber"].ToDecimalOrDefault(), "", r["ItemDescription"].ToString(), "", "", null, "");
        ...
        else
        {
            ...
            var lst = dc.usp_InvoiceQuota_Select(acBranch.Value.ToNullableInt(), null, "",
                acCustomer.Value.ToInt(), null, null, "", 1, null, null, 0).ToList();
            var lstById = lst.Where(x => x.ID < this.Invoice_ID).ToList();
            if (lstById.Count == 0 || dc.usp_Company_Select().First().IsOrder == false)
            {
                int Result = dc.usp_InvoiceQuota_Update(this.Invoice_ID, acBranch.Value.ToNullableInt(),
                       ...
                       txtUserRefNo.Text, this.DocRandomString, acSalesRep.Value.ToNullableInt(), acCashAccount.Value.ToNullableInt(),
                       txtDateFinish.Text.ToDate(), txtDeleveyDay.Text.ToNullableInt(), txtPaymentMethode.Text, txtAccountNumber.Text);
                ...
                if (Result > 0)
                {
                    foreach (DataRow r in this.dtItems.Rows)
                    {
                        if (r.RowState == DataRowState.Added)
                        {
                            ...
                            dc.usp_InvoiceQuotaDetailsWithDesciption_Insert(this.Invoice_ID, r["Store_ID"].ToInt(), r["Item_ID"].ToInt(), r["UnitCost"].ToDecimal(), r["Quantity"].ToDecimal(), r["Uom_iD"].ToInt(), r["Batch_ID"].ToNullableInt(), r["Tax_ID"].ToNullableInt(), r["PercentageDiscount"].ToDecimalOrDefault(), r["CashDiscount"].ToDecimalOrDefault(), r["TotalTax"].ToDecimalOrDefault(), r["Notes"].ToExpressString(), r["QtyInNumber"].ToDecimalOrDefault(), r["Capacity"].ToString(), r["ItemDescription"].ToString(), "", "", null, "");

                        }
                        if (r.RowState == DataRowState.Modified)
                        {
                            dc.usp_InvoiceQuotaDetails_Update(r["ID"].ToInt(), r["Store_ID"].ToInt(), r["Item_ID"].ToInt(), r["UnitCost"].ToDecimal(), r["Quantity"].ToDecimal(), r["Uom_iD"].ToInt(), r["Batch_ID"].ToNullableInt(), r["Tax_ID"].ToNullableInt(), r["PercentageDiscount"].ToDecimalOrDefault(), r["CashDiscount"].ToDecimalOrDefault(), r["TotalTax"].ToDecimalOrDefault(), r["Notes"].ToExpressString(), r["QtyInNumber"].ToDecimalOrDefault());
                        }
                        if (r.RowState == DataRowState.Deleted)
                        {
                            dc.usp_InvoiceQuotaDetails_Delete(r["ID", DataRowVersion.Original].ToInt());
                        }
                    }
        ...
        UserMessages.Message(null, Resources.UserInfoMessages.OperationSuccess + Serial, PageLinks.Quota + "?ID=" + this.Invoice_ID.ToExpressString(), PageLinks.QuotaList, PageLinks.Quota);
        return true;

    }

    private void FillInvoiceQuota()
    {
        var company = dc.usp_Company_Select().First();
        var invoice = dc.usp_InvoiceQuota_SelectByID(this.Invoice_ID).FirstOrDefault();
        ...
        txtAccountNumber.Text = invoice.AccountNumber;
        txtDateFinish.Text = invoice.DateFinish.ToExpressString();
        txtDeleveyDay.Text = invoice.DeliveryDays.ToExpressString();
        //txtPaymentMethode.Text - invoice.PaymentMethode.ToExpressString();
        ...
        btnPrint.Visible = MyContext.PageData.IsPrint; btnPrintArabic.Visible = MyContext.PageData.IsPrint;
        //btnPrintInventoryOrder.Visible = MyContext.PageData.IsPrint;
        pnlAddItem.Visible = (invoice.DocStatus_ID == 1);
        // btnCancel.Visible = ((invoice.DocStatus_ID == 1) || (invoice.DocStatus_ID == 2 && company.ReserveQty.Value)) && MyContext.PageData.IsDelete;
        ...
        this.dtItems = dc.usp_InvoiceQuotaDetailsWithDescription_Select(this.Invoice_ID).CopyToDataTable();

        this.dtTaxes = dc.usp_DocuemntTaxes_Select(DocumentsTableTypes.InvoiceQuota.ToInt(), this.Invoice_ID, false).CopyToDataTable();
        this.BindItemsGrid();
        gvTaxes.DataSource = this.dtTaxes;
        gvTaxes.DataBind();
    ...
    private void Calculate()
    {
        ...
        foreach (DataRow r in this.dtItems.Rows)
        {
            ...
            var uc = r["UnitCost"].ToDecimalOrDefault();
            var taxUc = r["UnitCost"].ToDecimalOrDefault() * taxInclude;
