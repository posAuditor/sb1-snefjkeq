...
public partial class Sales_OutputList : UICulturePage
{
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            doc.Load(Server.MapPath("~\\Reports\\Invoice_Inflamation_Print.rpt"));
            doc.SetParameterValue("@Invoice_ID", this.Invoice_ID);
    ...
    {
        byte EntryType = (byte)10;
        lnkadd.NavigateUrl = Request.PathInfo == "/Invoice" ? PageLinks.Invoice : "~/Sales/InvoiceOutput.aspx";
        ...
        var lstInvoices =
            dc.usp_InvoiceEmp_Select(acBranch.Value.ToNullableInt(), Currency_ID, txtSerialsrch.TrimmedText,
               acCustomerName.Value.ToNullableInt(), txtDateFromSrch.Text.ToDate(), txtDateToSrch.Text.ToDate(),
               txtUserRefNo.Text, DocStatus_ID, MyContext.CurrentCulture.ToByte(), HasInvoice, EntryType, MyContext.UserProfile.Contact_ID).ToList();



        ///.Where(x => x.NotStocked != 0);
