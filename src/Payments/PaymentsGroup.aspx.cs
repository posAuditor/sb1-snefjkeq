...
public partial class Payments_PaymentsGroup : UICulturePage
{
    ...
    private int PaymentDetails_ID
    {
        get
        {
            if (ViewState["PaymentDetails_ID"] == null) return 0;
            return (int)ViewState["PaymentDetails_ID"];
        }

        set
        {
            ViewState["PaymentDetails_ID"] = value;
        }
    }
    ...
    private void RefillForm(string strValue)
    {
        ...
        {
            this.Payment_ID = strValue.ToInt();
    ...
    protected void btnAddDetail_click(object sender, EventArgs e)
    {
        try
        {
            if (acCreditAccount.HasValue)
            {
                var contact_ID = dc.fun_GetContactFromChartOfAccount(acCreditAccount.Value.ToIntOrDefault());
                if (this.EditMode)
                {
                    this.dtPaymentDetailsGroup = dc.usp_PaymentsDetailsGroup_Select(PaymentDetails_ID).CopyToDataTable();
                    gvItems.DataSource = this.dtPaymentDetailsGroup;
                    gvItems.DataBind();
                ...
                {
                    var list = dc.usp_InvoicePayment(false, contact_ID).ToList();

                    DataTable dt = new DataTable();
                    dt.Columns.Add("ID", typeof(int));
                    dt.Columns.Add("Invoice_ID", typeof(int));
                    dt.Columns.Add("Serial", typeof(string));
                    dt.Columns.Add("OperationDate", typeof(DateTime));
                    dt.Columns.Add("TotalAmount", typeof(decimal));
                    dt.Columns.Add("InvoiceTotalAmount", typeof(decimal));
                    dt.Columns.Add("AmountPay", typeof(decimal));

                    foreach (var item in list)
                    {
                        DataRow rw = dt.NewRow();
                        rw[0] = item.ID;
                        rw[1] = item.Invoice_ID;
                        rw[2] = item.Serial;
                        rw[3] = item.OperationDate;
                        rw[4] = item.TotalAmount;
                        rw[5] = item.InvoiceTotalAmount;
                        rw[6] = item.AmountPay;
                    }
                    dtPaymentDetailsGroup = dt;
                    gvItems.DataSource = this.dtPaymentDetailsGroup;
                    gvItems.DataBind();
                }
                ...
                foreach (GridViewRow gvRow in gvItems.Rows)
                {
                    ...
                    decimal InvoiceTotalAmount = gvItems.DataKeys[gvRow.RowIndex]["InvoiceTotalAmount"].ToInt();
                    decimal TotalAmount = gvItems.DataKeys[gvRow.RowIndex]["TotalAmount"].ToInt();
                    ((TextBox)gvRow.FindControl("txtAmountPay")).Text = gvItems.DataKeys[gvRow.RowIndex]["AmountPay"].ToExpressString();
                ...
                mpeInvoiceDistribute.Show();
            }
            else
            {
                UserMessages.Message(this.MyContext.PageData.PageTitle, "أرجو إختيار حساب العميل", string.Empty);
            }
    ...
    protected void gvDetails_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    {
        try
        {
            ...
            this.ShowBalances();
            this.FilterSalesReps();
            this.PaymentDetails_ID = this.EditID;
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            UserMessages.Message(null, Resources.UserInfoMessages.OperationSuccess, PageLinks.PaymentsGroup + Request.PathInfo + "?ID=" + this.Payment_ID.ToExpressString(), PageLinks.PaymentsList + Request.PathInfo, PageLinks.PaymentsGroup + Request.PathInfo);
        }
        catch (Exception ex)
        {
            Logger.LogError(Resources.UserInfoMessages.OperationFailed, ex);
    ...
    protected void btnCancelApprove_Click(object sender, EventArgs e)
    {
        try
        {
            dc.usp_CancelApprove_PaymentForGroup(this.DocumentTableType_ID, this.Payment_ID);
            UserMessages.Message(null, Resources.UserInfoMessages.OperationSuccess, PageLinks.PaymentsGroup + Request.PathInfo + "?ID=" + this.Payment_ID.ToExpressString(), PageLinks.PaymentsList + Request.PathInfo, PageLinks.PaymentsGroup + Request.PathInfo);
    ...
    protected void btnDistibute_Click(object sender, EventArgs e)
    {
    ...
    protected void LinkButton1_Click(object sender, System.EventArgs e)
    {
        ...
        this.FillItemsPayment(sender);
        this.PaymentDetails_ID = 0;
    }
    private void FillItemsPayment(object sender, int typeX = 0)
    {

        int Index = 0;
        if (typeX == 0)
        {
            Index = ((GridViewRow)((LinkButton)sender).Parent.Parent).RowIndex;
        }
        else
        {
            Index = ((GridViewRow)((ABFTextBox)sender).Parent.Parent).RowIndex;
        }

        //var Invoice_ID = gvItems.DataKeys[Index]["ID"].ToInt();
        decimal valueAmountPay = 0;
        foreach (GridViewRow gvRow in gvItems.Rows)
        {
            valueAmountPay += ((TextBox)gvRow.FindControl("txtAmountPay")).Text.ToDecimalOrDefault();
        }
        foreach (GridViewRow gvRow in gvItems.Rows)
        {
            if (gvRow.RowIndex == Index)
            {
                if (((TextBox)gvRow.FindControl("txtAmountPay")).Text != string.Empty || typeX == 0)
                {
                    decimal InvoiceTotalAmount = gvItems.DataKeys[gvRow.RowIndex]["InvoiceTotalAmount"].ToDecimalOrDefault();
                    decimal TotalAmount = gvItems.DataKeys[gvRow.RowIndex]["TotalAmount"].ToDecimalOrDefault();
                    //((TextBox)gvRow.FindControl("txtAmountPay")).Text = (Math.Round(InvoiceTotalAmount, 2) - Math.Round(TotalAmount, 2)).ToExpressString();
                    //if (valueAmountPay + ((TextBox)gvRow.FindControl("txtAmountPay")).Text.ToDecimalOrDefault() > txtAmount.Text.ToDecimalOrDefault())
                    //{
                    //    ((TextBox)gvRow.FindControl("txtAmountPay")).Text = ((Math.Round(txtAmount.Text.ToDecimalOrDefault(), 2)) - Math.Round(valueAmountPay, 2)).ToExpressString() ;
                    //}
                }
                else
                {
                    ((TextBox)gvRow.FindControl("txtAmountPay")).Text = string.Empty;
                }
            }
        }
    ...
    protected void txtAmountPay_TextChanged(object sender, System.EventArgs e)
    {
        this.FillItemsPayment(sender, 1);
    }
    protected void LinkButton1_Click1(object sender, System.EventArgs e)
    {

    }
    protected void lnkClear_Click(object sender, System.EventArgs e)
    {
    ...
    protected void btnDistribute_Click(object sender, System.EventArgs e)
    {
        decimal valueAmountPay = 0;
        foreach (GridViewRow gvRow in gvItems.Rows)
        {
        ...
        if (valueAmountPay != txtAmount.Text.ToDecimalOrDefault())
        {
            UserMessages.Message(this.MyContext.PageData.PageTitle, "مجموع  البمالغ غير مطابق مع المبلغ الاساسي", string.Empty);
            mpeInvoiceDistribute.Show();
        }
        else
        {
            ...
            foreach (GridViewRow gvRow in gvItems.Rows)
            {
                int id = gvItems.DataKeys[gvRow.RowIndex]["ID"].ToInt();
                int invoice_ID = gvItems.DataKeys[gvRow.RowIndex]["Invoice_ID"].ToInt();
                var IsAdd = false;

                var objCast = (usp_PaymentsDetails_SelectResult)gvRow.DataItem;
                DataRow rGroup = null;
                if (id == 0)
                {
                    rGroup = this.dtPaymentDetailsGroup.NewRow();
                    if (invoice_ID > 0)
                    {
                        rGroup = this.dtPaymentDetailsGroup.Select("Invoice_ID=" + invoice_ID)[0];
                        IsAdd = true;
                    }
                    else
                        rGroup["ID"] = this.dtPaymentDetailsGroup.GetID("ID");
                }
                else
                {
                    rGroup = this.dtPaymentDetailsGroup.Select("ID=" + id)[0];
                }
                rGroup["InvoiceTotalAmount"] = gvItems.DataKeys[gvRow.RowIndex]["InvoiceTotalAmount"];
                rGroup["TotalAmount"] = gvItems.DataKeys[gvRow.RowIndex]["TotalAmount"];
                rGroup["AmountPay"] = ((TextBox)gvRow.FindControl("txtAmountPay")).Text.ToDecimalOrDefault();
                rGroup["Invoice_ID"] = gvItems.DataKeys[gvRow.RowIndex]["Invoice_ID"];

                if (id == 0 && !IsAdd) this.dtPaymentDetailsGroup.Rows.Add(rGroup);
            }
    ...
    protected void btnReDistribute_Click(object sender, System.EventArgs e)
    {
        if (acCreditAccount.HasValue)
        {
            var contact_ID = dc.fun_GetContactFromChartOfAccount(acCreditAccount.Value.ToIntOrDefault());
            this.dtPaymentDetailsGroup = dc.usp_InvoicePayment(false, contact_ID).CopyToDataTable();
            gvItems.DataSource = this.dtPaymentDetailsGroup;
            gvItems.DataBind();
        }
        mpeInvoiceDistribute.Show();
    ...
    private void FilterByBranchAndCurrency()
    {
        try
        {
            ...
            acCreditAccount.ContextKey = this.MyContext.CurrentCulture.ToByte().ToExpressString() + "," + acBranch.Value + "," + ddlCurrency.SelectedValue + "," + CreditParent_ID + "," + IncludeParent_credit.ToExpressString();
            // acCreditAccount.ContextKey = "" + acBranch.Value + "";

            acDebitAccount.ContextKey = this.MyContext.CurrentCulture.ToByte().ToExpressString() + "," + acBranch.Value + "," + ddlCurrency.SelectedValue + "," + DebitParent_ID + "," + IncludeParent_debit.ToExpressString();
    ...
    private bool Save(bool IsApproving, System.Data.Common.DbTransaction trans)
    {
        ...
        if (!this.EditMode)
        {
            this.Payment_ID = dc.usp_Payments_Insert(txtOperationDate.Text.ToDate(), DateTime.Now, MyContext.UserProfile.Contact_ID, approvedBY_ID, ApproveDate, txtUserRefNo.TrimmedText, ref Serial, this.Serial_ID, txtNotes.Text, this.Total, acCostCenter.Value.ToNullableInt(), DocStatus_ID, this.EntryType, acBranch.Value.ToNullableInt(), txtRatio.Text.ToDecimal(), ddlCurrency.SelectedValue.ToInt(), this.RelatedDoc_ID, this.RelatedDocTableType_ID, this.DocRandomString);
            if (this.Payment_ID > 0)
            {
                foreach (DataRow r in this.dtPaymentDetails.Rows)
                {
                    if (r.RowState == DataRowState.Deleted) continue;
                    var IdPaymentDetails = dc.usp_PaymentsDetailsDiscount_Insert(this.Payment_ID, r["DebitAccount_ID"].ToInt(), r["CreditAccount_ID"].ToInt(), r["Amount"].ToDecimal(), r["SalesRep_ID"].ToNullableInt(), r["Notes"].ToExpressString(), r["CostCenter_ID"].ToNullableInt(), r["Discount"].ToDecimalOrDefault(), false, null, 0, null, r["PaymentType"].ToInt(),null,null);
                    foreach (DataRow item in this.dtPaymentDetailsGroup.Rows)
                    {
                        if (item["AmountPay"].ToDecimalOrDefault() > 0)
                            dc.usp_PaymentsDetailsGroup_CRUD(item["ID"].ToInt(), item["Invoice_ID"].ToInt(), IdPaymentDetails, item["TotalAmount"].ToDecimalOrDefault(), item["InvoiceTotalAmount"].ToDecimalOrDefault(), item["AmountPay"].ToDecimalOrDefault());
                        else
                        {
                    ...
                    if (IsApproving)
                    {
                        foreach (DataRow item in this.dtPaymentDetailsGroup.Rows)
                        {
                            if (item["AmountPay"].ToDecimalOrDefault() > 0)
                            {
                                int PayGroup = dc.usp_PaymentsForGrou_Insert(txtOperationDate.Text.ToDate(), DateTime.Now, MyContext.UserProfile.Contact_ID, approvedBY_ID, ApproveDate, txtUserRefNo.TrimmedText, ref Serial, this.Serial_ID, txtNotes.Text, item["AmountPay"].ToDecimalOrDefault(), acCostCenter.Value.ToNullableInt(), 2, 18, acBranch.Value.ToNullableInt(), txtRatio.Text.ToDecimal(), ddlCurrency.SelectedValue.ToInt(), item["Invoice_ID"].ToInt(), 1, DateTime.Now.Ticks.ToExpressString() + "_" + Guid.NewGuid().ToExpressString(), this.Payment_ID);
                                if (PayGroup > 0)
                                {


                                    dc.usp_PaymentsDetailsDiscount_Insert(PayGroup, r["DebitAccount_ID"].ToInt(), r["CreditAccount_ID"].ToInt(), item["AmountPay"].ToDecimalOrDefault(), null, r["Notes"].ToExpressString(), r["CostCenter_ID"].ToNullableInt(), r["Discount"].ToDecimalOrDefault(), false, null,0, null, r["PaymentType"].ToInt(), null, null);
        ...
        {
            int Result = dc.usp_Payments_Update(this.Payment_ID, txtOperationDate.Text.ToDate(), approvedBY_ID, ApproveDate, txtUserRefNo.TrimmedText, ref Serial, this.Serial_ID, txtNotes.Text, this.Total, acCostCenter.Value.ToNullableInt(), DocStatus_ID, acBranch.Value.ToNullableInt(), txtRatio.Text.ToDecimal(), ddlCurrency.SelectedValue.ToInt());
            if (Result > 0)
            {
                foreach (DataRow r in this.dtPaymentDetails.Rows)
                {
                    ...
                    {
                        dc.usp_PaymentsDetailsDiscount_Insert(this.Payment_ID, r["DebitAccount_ID"].ToInt(), r["CreditAccount_ID"].ToInt(), r["Amount"].ToDecimal(), r["SalesRep_ID"].ToNullableInt(), r["Notes"].ToExpressString(), r["CostCenter_ID"].ToNullableInt(), r["Discount"].ToDecimal(), false, null,0, null, r["PaymentType"].ToInt(), null, null);
                    }
                    if (r.RowState == DataRowState.Modified)
                    {
                        dc.usp_PaymentsDetailsDiscount_Update(r["ID"].ToInt(), r["DebitAccount_ID"].ToInt(), r["CreditAccount_ID"].ToInt(), r["Amount"].ToDecimal(), r["SalesRep_ID"].ToNullableInt(), r["Notes"].ToExpressString(), r["CostCenter_ID"].ToNullableInt(), r["Discount"].ToDecimal(), false, null, 0, null, r["PaymentType"].ToInt(), null, null);
                    }
                    ...
                    {
                        if (item["AmountPay"].ToDecimalOrDefault() > 0)
                            dc.usp_PaymentsDetailsGroup_CRUD(item["ID"].ToInt(), item["Invoice_ID"].ToInt(), r["ID"].ToInt(), item["TotalAmount"].ToDecimalOrDefault(), item["InvoiceTotalAmount"].ToDecimalOrDefault(), item["AmountPay"].ToDecimalOrDefault());
                        else
                        {
                    ...
                    if (IsApproving)
                    {
                        if (r["CreditAccount_ID"].ToInt() > 0)
                        {
                            var contact_ID = dc.fun_GetContactFromChartOfAccount(r["CreditAccount_ID"].ToInt());
                            this.dtPaymentDetailsGroup = dc.usp_PaymentsDetailsGroup_Select(r["ID"].ToInt()).CopyToDataTable();
                        }
                        ...
                        foreach (DataRow item in this.dtPaymentDetailsGroup.Rows)
                        {
                            if (item["AmountPay"].ToDecimalOrDefault() > 0)
                            {
                                ...
                                if (PayGroup > 0)
                                {
                                    dc.usp_PaymentsDetailsDiscount_Insert(PayGroup, r["DebitAccount_ID"].ToInt(), r["CreditAccount_ID"].ToInt(), item["AmountPay"].ToDecimalOrDefault(), r["SalesRep_ID"].ToNullableInt(), r["Notes"].ToExpressString(), r["CostCenter_ID"].ToNullableInt(), r["Discount"].ToDecimal(), false, null, 0, null, r["PaymentType"].ToInt(), null, null);
        ...
        UserMessages.Message(null, Resources.UserInfoMessages.OperationSuccess + Serial, PageLinks.PaymentsGroup + Request.PathInfo + "?ID=" + this.Payment_ID.ToExpressString(), PageLinks.PaymentsGroupList + Request.PathInfo, PageLinks.PaymentsGroup + Request.PathInfo);
        return true;
    }
    ...
    private void SetPageWorkingMode()
    {
        ...
        switch (Request.PathInfo)
        {
            case "/CashInCustomer":
                ...
                this.EntryType = PaymentsTypes.CashInCustomerGroup.ToByte();
