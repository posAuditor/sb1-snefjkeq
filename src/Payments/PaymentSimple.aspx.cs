...
public partial class Payments_PaymentSimple : UICulturePage
{
    ...
    protected void btnAddDetail_click(object sender, EventArgs e)
    {
        try
        {
            ...
            r["PaymentType"] = ddlPaymentMethode.SelectedValue.ToIntOrDefault();
            r["MethodeName"] = ddlPaymentMethode.SelectedItem.ToExpressString();
            if (ddlPaymentMethode.SelectedValue.ToInt() >= 0)
            {
                ...
                string IDPM = ddlPaymentMethode.SelectedValue;//.ToInt();
                var id = int.Parse(IDPM);
                var ChoosedPayMethodes_ID = paymentMethods.Where(c => c.ID == id).FirstOrDefault();
                ...
                acDebitAccount.Value = Accou_ID.ToExpressString();
                r["DebitAccount_ID"] = Accou_ID;
            }
    ...
    private void LoadControls()
    {
        ...
        var listSos = dc.SettingPointOs.Where(c => c.IsActive.Value).ToList().Select(c =>
            new PaymentMethodeCls
            {
                Name = c.Name,
                ID = c.ID
            }).ToList();
    ...
    private bool Save(bool IsApproving, System.Data.Common.DbTransaction trans)
    {
        ...
        UserMessages.Message(null, Resources.UserInfoMessages.OperationSuccess + Serial, PageLinks.PaymentSimple + Request.PathInfo + "?ID=" + this.Payment_ID.ToExpressString(), PageLinks.PaymentSimpleList + Request.PathInfo, PageLinks.PaymentSimple + Request.PathInfo);
        return true;
    }
    ...
    private void SetPageWorkingMode()
    {
        ...
        switch (Request.PathInfo)
        {
            ...
            case "/ReceiptNots":
                this.Serial_ID = DocSerials.ReceiptNotes.ToInt();
                this.DocumentTableType_ID = DocumentsTableTypes.Payment_CashIn.ToInt();
                this.EntryType = PaymentsTypes.ReceiptNotes.ToByte();
                this.OperationType_ID = OperationTypes.CashIn.ToInt();
                this.lblTitle.Text = Resources.Labels.ReceiptNotes;
                break;
    ...
    private void InsertOperation()
    {
        ...
        if (this.EntryType == PaymentsTypes.ReceiptNotes.ToByte())
        //حسم الممنوح
        {
